...
const isFormArray = (prev: any, curr: any): boolean =>
  isArray(curr) || isArray(prev);
const isFormGroup = (prev: any, curr: any): boolean =>
  isObject(curr) || isObject(prev);
const isFormControl = (prev: any, curr: any): boolean =>
  !isFormArray(prev, curr) && !isFormGroup(prev, curr);
const convertTypesToArray = (left: any, right: any): Array<[]> => [
  left as unknown as [],
  right as unknown as [],
];
 ...
 * @source https://github.com/ngneat/reactive-forms/blob/master/libs/reactive-forms/src/lib/operators/diff.ts
 */
...
function compareFormGroup<T>(prev: T, curr: T): T | undefined {
  const reduced = reduceFormGroup(prev as any, curr);
